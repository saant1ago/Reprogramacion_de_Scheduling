...
num1,num2 = st.slider('Rango de horas de entrega de los nuevos pedidos:',7,17,(7,13))
rango_horas[0]=num1
rango_horas[1] = num2
st.write('Los nuevos pedidos llegarán de',str(rango_horas[0]),'a',str(rango_horas[1]),'horas')


num3,num4 = st.slider('Rango de la cantidad de producto de los pedidos:',50,500,(50,400),step=50)
rango_cantidad[0]=  num3
rango_cantidad[1] = num4+1
st.write('Los nuevos pedidos llegarán con una cantidad de',str(rango_cantidad[0]),'a',str(rango_cantidad[1]),'de producto')


new_la = st.slider('Media de llegada de los pedidos',1,5)
lamb = new_la
st.write('La media de llegadas será de:',str(lamb))
  

proba = st.slider('Probabilidad de eliminar un pedido',0.0,1.0)
prob = proba
st.write('La probabilidad de que se elimine un pedido será de:',str(prob))

   
#if option == 'Media de llegada':
   #st.slider
...
if st.session_state.clicked == True:
    ...
    pedidos_iniciales = [
        ...
        for i, row in pedidos.iterrows()
    ]

    # Crear la simulación
    simulacion = GestorPedidos(pedidos_iniciales)

    
    #hacer modificaciones
    modificaciones = simulacion.simular_cambio(lam=lamb, prob=proba)
    
    st.write('Pedidos Agregados:')

    for agregado in modificaciones['agregados']:
        st.write(agregado)

    st.write('\nPedidos Eliminados:', ', '.join(map(str, modificaciones['eliminados'])))

    df=reprogamar(pedidos, rates, df, modificaciones)
    ...
    with col2:
        ...
        st.data_editor(pedidos,key='2')

    st.subheader('Cronograma Reprogramado')
    st.plotly_chart(generar_gantt_2(df))
    
    with st.expander('Comparación con cronograma inicial'):
       st.plotly_chart(generar_gantt('df.csv'),key='final')
